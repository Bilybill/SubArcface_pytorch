...
__all__ = ["regnetx_400m", "regnety_400m"]
...
class AnyHead(nn.Module):
    ...
    def __init__(self, w_in, nc):
        ...
        self.fc = nn.Linear(w_in * 7 * 7, nc)
        self.bn2 = nn.BatchNorm1d(nc, affine=False)
...
class RegNet(AnyNet):
    ...
    def __init__(self, feature_dim, cfg, bn=None):
...
def regnetx_400m(feature_dim):
    model = RegNet(feature_dim, regnetX_400M_config)
    return model


def regnety_400m(feature_dim):
    model = RegNet(feature_dim, regnetY_400M_config)
    return model
